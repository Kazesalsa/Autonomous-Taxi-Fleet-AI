...
def main():
    ...
    pygame.display.set_caption("AI Taxi Fleet Management - LoS & Fog of War")
    ...
    while running:
        if not is_paused:
            try:
                ...
                # Quản lý hết hạn vạch qua đường an toàn
                edges_to_remove = []
                ...
                for e_id in edges_to_remove:
                    if e_id in broken_edges:
                        ...
                        # CHỈ đánh thức xe đang chờ ở đúng vạch này
                        for v in vehicles:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT: running = False
                ...
                try:
                    ...
                    if action == "pause": is_paused = not is_paused
                        ...
                        if action[0] == "REMOVE_BLOCK":
                            ...
                            if edge_id in broken_edges:
                                ...
                                for v in vehicles:
                                    if v.state in ["STUCK_AT_OBSTACLE", "WAIT_U_TURN"]:
                                        p = online_search_replanning(graph, v.current_edge_start_id, v.target_node_id, v.final_goal_id, broken_edges)
                                        if p:
                                            v.state = "MOVING"
                                            v.target_node_id = p[1] if len(p) > 1 else None
                                            v.path = p[2:] if len(p) > 2 else []
                                    elif v.state == "WAIT_CROSSWALK":
                                        curr_e = tuple(sorted((v.current_edge_start_id, v.target_node_id))) if v.target_node_id else None
                            ...
                            if select_mode == 'CUST_START' and 'P' not in action[1] and action[1] != 'GATE':
                                ...
                                goal_n = 'HOSPITAL' # Luôn hướng về Bệnh viện
                    ...
                    print(f"Try-Catch đã bảo vệ hệ thống khỏi crash: {e}")

        if not is_paused:
            try:
                # 1. Cảm biến Tầm nhìn (LiDAR Line of Sight)
                for v in vehicles:
                ...
                # 5. Cập nhật vị trí & Nhiệm vụ Taxi
                for v in vehicles[:]:
