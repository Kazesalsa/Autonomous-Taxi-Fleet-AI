...
def run_weighted_a_star(context) -> ExperimentResult:
    ...
    open_set = []
    start_h = h_fn(graph.nodes[start_id], goal_node) * w
    ...
    while open_set:
        ...
        if curr_id == goal_id:
            ...
            return ExperimentResult("Weighted A*", "Informed", True, exec_time, {"nodes_expanded": nodes_expanded, "cost": g_score[goal_id]}, path)
        ...
        for edge in graph.nodes[curr_id].edges:
            ...
            if tentative_g < g_score.get(neighbor_id, float('inf')):
                ...
                f_score = tentative_g + (h_fn(graph.nodes[neighbor_id], goal_node) * w)
                heapq.heappush(open_set, (f_score, neighbor_id))

    exec_time = (time.perf_counter() - start_time) * 1000
    return ExperimentResult("Weighted A*", "Informed", False, exec_time, {"nodes_expanded": nodes_expanded})
